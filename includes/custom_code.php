   ...
   // echo '<pre>'; print_r($new_events); exit;
	...
	<div class="wrap">
        ...
        <script>
            ...
            function update_user_address(val, post_id)
            {
                jQuery.ajax({
                    type : "POST",
                    url : "/wp-admin/admin-ajax.php",
                    data : {action: "update_user_address",
                            val: val, 
                            post_id: post_id
                        },
                    beforeSend: function()
                    {
                        jQuery('#cd_user_address'+post_id).parent().parent().css('opacity', '0.4');
                    },      
                    success: function(response) 
                    {
                        var data = JSON.parse(response);
                        jQuery('#cd_user_address'+post_id).parent().parent().css('opacity', '1');
                        if(data.status != 'success')
                        {
                            alert('Error! Something wrong, please refresh page and try again')
                        }
...
function update_user_address()
{
    if(isset($_POST['val']) && $_POST['post_id'] != '')
	{
        update_post_meta($_POST['post_id'], 'address', $_POST['val']);
        echo json_encode(array('status' =>  'success'));
	}
    else
    {
